...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAADBQADywIAAmYGoVeNbkxxAAEGOAUC')
            return
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>📂𝙼𝙾𝚅𝙸𝙴 𝙽𝙰𝙼𝙴 :- {search} ‌ </b>\n<b>POWERED BY AMIGO CINEMAS</b>", reply_markup=InlineKeyboardMarkup(buttons))

            else:
                await message.reply_text(f"<b>📂𝙼𝙾𝚅𝙸𝙴 𝙽𝙰𝙼𝙴 :- {search}  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
        ...
        if API_KEY:
            poster=await get_poster(search)
        if poster:
            await message.reply_photo(photo=poster, caption=f"<b>📂𝙼𝙾𝚅𝙸𝙴 𝙽𝙰𝙼𝙴 :- {search} ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
        else:
